...
void LCD_ST7032_Init(ST7032_InitTypeDef *hlcd)
{
	...
	HAL_Delay(CMD_DELAY);
	LCD_buff[0] = DISP_CMD;
	LCD_buff[1] = FUNC_SET_TBL1;
	LCD_buff[2] = 0x14;	//Internal OSC frequency
	LCD_buff[3] = 0x78;	//Contrast (low-bits) set
	LCD_buff[4] = 0x5E;	//Power/ICON control/Contrast (high-bits) set
	LCD_buff[5] = 0x6D;	//Follower control
...
void LCD_shift(uint8_t sc_rl_s,uint8_t shift_time)
{
	...
	for(int i = 0; i < shift_time; i++)
	{
		...
		{
		case ST7032_SR:
			LCD_command(DISP_SHIFT_CMD | ST7032_SR);
			break;
		case ST7032_SL:
			LCD_command(DISP_SHIFT_CMD | ST7032_SL);
			break;
		case ST7032_CR:
			LCD_command(DISP_SHIFT_CMD | ST7032_CR);
			break;
		case ST7032_CL:
			LCD_command(DISP_SHIFT_CMD | ST7032_CL);
			break;
		}
	}
